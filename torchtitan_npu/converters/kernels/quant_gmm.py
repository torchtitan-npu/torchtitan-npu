...
class GMMFunctionMxfp8(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, group_list):
        ...
        return torch_npu.npu_grouped_matmul([x_mxfp8], [weight_mxfp8], bias=None,
                                            scale=[weight_scale],
                                            per_token_scale=[x_scale], group_list=group_list, group_type=0,
                                            output_dtype=x.dtype, group_list_type=0,
                                            scale_dtype=torch_npu.float8_e8m0fnu,
                                            per_token_scale_dtype=torch_npu.float8_e8m0fnu, split_item=3)[0]
    ...
    def backward(ctx, grad):
        ...
        grad_input = torch_npu.npu_grouped_matmul([grad_mxfp8], [rearrange(weight_mxfp8, 'n h f -> n f h')], bias=None,
                                            scale=[rearrange(weight_scale, 'n h f g -> n f h g')],
                                            per_token_scale=[grad_scale], group_list=group_list, group_type=0,
                                            output_dtype=grad.dtype, group_list_type=0,
                                            scale_dtype=torch_npu.float8_e8m0fnu,
                                            per_token_scale_dtype=torch_npu.float8_e8m0fnu, split_item=3)[0]
        
        
        x_mxfp8, x_scale = torch_npu.npu_grouped_dynamic_mx_quant(
            x, group_list.to(torch.int32), round_mode="rint", dst_type=torch.float8_e4m3fn, blocksize=32)
        grad_mxfp8, grad_scale = torch_npu.npu_grouped_dynamic_mx_quant(
            grad, group_list.to(torch.int32), round_mode="rint", dst_type=torch.float8_e4m3fn, blocksize=32)
        grad_weight = torch_npu.npu_grouped_matmul([x_mxfp8.t()], [grad_mxfp8], bias=None,
                                            scale=[grad_scale],
                                            per_token_scale=[rearrange(x_scale, 'n h f -> h n f')],
                                            group_list=group_list, group_type=2,
                                            output_dtype=x.dtype, group_list_type=0,
                                            scale_dtype=torch_npu.float8_e8m0fnu,
                                            per_token_scale_dtype=torch_npu.float8_e8m0fnu, split_item=3)[0]
...
class GMMFunctionHif8(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, group_list):
        ...
        return torch_npu.npu_grouped_matmul([x_quant], [w_quant], scale=[w_scale], per_token_scale=[x_scale],
                                            group_list=group_list, group_type=0, bias=None, split_item=3,
                                            output_dtype=x.dtype, group_list_type=0, **gmm_kwargs)[0]
    ...
    def backward(ctx, grad):
        ...
        grad_input = torch_npu.npu_grouped_matmul([grad_quant], [w_quant], bias=None,
                                            scale=[w_scale], per_token_scale=[grad_scale],
                                            group_list=group_list, group_type=0, split_item=3,
                                            output_dtype=grad.dtype, group_list_type=0, **gmm_kwargs)[0]
        
        x_quant, x_scale, grad_quant, grad_scale = GMMFunctionHif8.quantize(
            x, grad, torch_npu.hifloat8, torch_npu.hifloat8
        )
        grad_weight = torch_npu.npu_grouped_matmul(
            [x_quant.t()], [grad_quant],
            scale=[grad_scale], per_token_scale=[x_scale],
            group_list=group_list, group_type=2, bias=None, split_item=3,
            output_dtype=x.dtype, group_list_type=0, **gmm_kwargs
        )[0]

        return grad_input, grad_weight, None
    
    @staticmethod
    def quantize(x: torch.Tensor, weight: torch.Tensor, x_dst_type: torch.dtype, w_dst_type: torch.dtype):
