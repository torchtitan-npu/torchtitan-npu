...
import torch
import torch_npu
from torchtitan_npu.patches.quantization.quant_config import MXLinearRecipeName, MXLinearConfig 
...
class MXfp8MM(torch.autograd.Function):
    ...
    def backward(ctx, grads):
        ...
        dw = torch_npu.npu_quant_matmul(grads_mxfp8.t(), x_mxfp8, x_scale,
...
class Hif8MM(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        output = torch_npu.npu_quant_matmul(x_quant, w_quant.t(), w_scale, pertoken_scale=x_scale,
                                            output_dtype=x.dtype, **mm_kwargs)
